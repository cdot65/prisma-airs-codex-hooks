...
| `command` | Shell command to execute. The installer embeds the **absolute node binary** (`process.execPath` at install time) because Codex runs hooks with a system PATH — nvm/asdf-managed `node` is not on it and a bare `node` fails with exit 127. |
